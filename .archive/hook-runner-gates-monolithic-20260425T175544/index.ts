 ...
 *
 * 28 modules ported from hook-runner's CommonJS format to OpenClaw Plugin SDK.
 *
 * before_tool_call gates (18):
 ...
 *   no-unnecessary-sleep, claude-p-pattern, customer-data-gate
 *
...
// ── customer-data-gate ───────────────────────────────────────────────
// WHY: Coconut leaked customer PII (names, contacts, case numbers, account
// details) into Slack channels that don't allow customer data per
// DATA-POLICY.md. Incident: 2026-04-19 — task board update and Company 3
// prep posted to Slack with customer names, contact names, account counts,
// case numbers, and infra details. Joel caught it twice in two days.

// Channels where customer-specific data is BLOCKED
const CUSTOMER_BLOCKED_CHANNELS: Record<string, string> = {
...
// Joel DM — allows limited customer (urgent only) but blocks infra
const INFRA_ONLY_BLOCKED_CHANNELS: Record<string, string> = {
  "D0ATWPM4DTK": "Joel DM",
};

// All Slack channels where internal infra details are blocked
const INFRA_BLOCKED_CHANNELS: Record<string, string> = {
  ...CUSTOMER_BLOCKED_CHANNELS,
  ...INFRA_ONLY_BLOCKED_CHANNELS,
};

// Patterns that indicate customer-specific data
const CUSTOMER_DATA_PATTERNS: Array<{ name: string; re: RegExp }> = [
  // External email addresses (not @trendmicro.com)
  { name: "External email", re: /\b[A-Za-z0-9._%+-]+@(?!trendmicro\.com\b)[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/ },
  // Case/ticket numbers (Trend Micro support formats)
  { name: "Case/ticket number", re: /\b(?:SR|INC|CAS(?:E)?|TKT|TICKET)[-#: ]?\d{4,}\b/i },
  // AWS account IDs (12 digits, contextual)
  { name: "AWS account ID", re: /\b\d{12}\b(?=[\s\S]{0,40}(?:aws|account|tenant|cloud|disconnect))/i },
  // V1 tenant/customer UUIDs
  { name: "Tenant/customer UUID", re: /\b[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\b/i },
  // Phone numbers (US format)
  { name: "Phone number", re: /\b(?:\+?1[-. ]?)?\(?\d{3}\)?[-. ]?\d{3}[-. ]?\d{4}\b/ },
  // Known customer company names
  { name: "Customer name", re: /\b(?:Entertainment\s+Partners|Panavision|DeepSeas|Legato\s+Security|BBSI|CVS\s+Health|Company\s*3|Dole(?:\s+Food)?)\b/i },
  // Known customer contact names
  { name: "Customer contact", re: /\b(?:Dan\s+Toresi|Trilok\s+Somaraju|Terry\s+Bahr|Matt\s+Patterson|(?:Mark|Zoe)\s+Checklin|Cristian\s+Hamilton|Patrick\s+Joyce|Adam\s+Ryding|Craig\s+McLaughlin|Matt\s+Strickler|Julian\s+Harrington|Robert\s+Romero)\b/i },
  // MDR alert IDs / workbench IDs
  { name: "MDR/workbench ID", re: /\bWB-\d{4,}|\bMDR-\d{4,}|\bALERT-\d{4,}/i },
];

// Internal infrastructure patterns
const INFRA_PATTERNS: Array<{ name: string; re: RegExp }> = [
  // Internal Windows file paths
  { name: "Internal file path", re: /\/mnt\/c\/Users\//i },
  // Internal-only URLs
  { name: "Internal URL", re: /\b(?:support\.xdr|bleego\.productcloud)\.trendmicro\.com\b/i },
  // Confluence/wiki content
  { name: "Wiki/Confluence", re: /\b(?:confluence\.trendmicro|wiki\.trendmicro|sharepoint\.com\/sites\/)/i },
  // Internal CLI/config structures (policy.dat, DDEI internals)
  { name: "Internal config reference", re: /\bpolicy\.dat\b|\bDDEI\s+(?:config|export|CLI|command)/i },
];

function customerDataGate(toolName: string, params: Record<string, unknown>): string | null {
  // Intercept the message tool with action=send
  ...
  // Strip prefix like "channel:" or "user:"
  const target = rawTarget.includes(":") ? rawTarget.split(":").slice(1).join(":") : rawTarget;

  const channel = String(params.channel || "");
  // Only enforce on Slack (Teams is approved for customer data)
  if (channel && channel !== "slack") return null;
  ...
  // Check customer data patterns against restricted channels
  const customerBlockedName = CUSTOMER_BLOCKED_CHANNELS[target];
  if (customerBlockedName) {
    for (const pat of CUSTOMER_DATA_PATTERNS) {
  ...
  // Check infra patterns against all infra-blocked channels
  const infraBlockedName = INFRA_BLOCKED_CHANNELS[target];
  if (infraBlockedName) {
  ...
  const channelName = customerBlockedName || infraBlockedName || target;
  return (
    `CUSTOMER DATA BLOCKED for ${channelName}.\n` +
    `Detected: ${findings.join(", ")}\n\n` +
    `Per DATA-POLICY.md, this channel does not allow customer-specific data or internal infra details.\n` +
    `What to do instead:\n` +
    `  1. Post customer-specific details to Teams (approved channel for customer data)\n` +
    `  2. Remove names, contacts, case numbers, account details and post a sanitized summary\n` +
    `  3. For urgent customer flags only: use Joel DM with minimal references\n\n` +
    `Do NOT retry the same content. Rewrite without: ${findings.join(", ")}.`
  );
}
...
const beforeToolCallGates: Record<string, GateFunction> = {
  ...
  "customer-data-gate": customerDataGate,
};
...
export default definePluginEntry({
  ...
  description:
    "28 ported hook-runner gate modules — git safety, secret scanning, code quality, " +
    "commit hygiene, test coverage, disk safety, customer data protection, " +
    "AI behavioral guardrails, auto-continue enforcement, and session start reminders.",
