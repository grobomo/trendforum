...
1. **Anonymous auth via rotating shared password** — proves membership without revealing who you are
...
User enters WiFi password → Server validates against current month's hash
                          ...
                          → Token contains: { role: "member", exp: 24h, jti: random }
...
- Each post gets a random per-thread display name (e.g., "Anon-A7x") scoped to that post's comment tree only — so you can follow a conversation but can't correlate across posts
...
model Comment {
  ...
  displayName String    // random per-post-tree, e.g., "Anon-K9m"
  body        String
  score       Int       @default(0)
  createdAt   DateTime  @default(now())
  votes       Vote[]
...
Coconut is a built-in AI bot that monitors TrendForum:
